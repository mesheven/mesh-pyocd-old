...
class STM32F031(CortexM):

    memoryMap = MemoryMap(
        FlashRegion(    start=0x08000000,  length=0x8000,      blocksize=0x400, isBootMemory=True),
        RamRegion(      start=0x20000000,  length=0x1000)
        )

    def __init__(self, link):
        super(STM32F031, self).__init__(link, self.memoryMap)

    def init(self):
        logging.debug('stm32f031 init')
        CortexM.init(self)
